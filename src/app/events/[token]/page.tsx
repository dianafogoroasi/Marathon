...
function StatusBadge({ active, label }: { active: boolean; label: string }) {
  return (
    <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-medium ${
      active ? "bg-green-500/20 text-green-400 border border-green-500/30" : "bg-gray-700/50 text-gray-500 border border-gray-700"
    }`}>
      {active ? "✓" : "·"} {label}
    </span>
  );
...
function ParticipantModal({
  ...
  const [form, setForm] = useState<Partial<Participant>>(
    participant ?? { name: "", hasBib: false, hasTransport: false, hasHotel: false }
  );
      ...
      <div className="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-md shadow-2xl">
        <div className="p-6 border-b border-white/10">
          <h2 className="text-xl font-bold text-white">
            {participant ? "Modifica" : "Aggiungi partecipante"}
          </h2>
        </div>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-3">
            <p className="text-gray-400 text-sm font-medium">Stato logistico</p>

            <label className="flex items-center gap-3 cursor-pointer group">
              <input
                type="checkbox"
                className="w-4 h-4 accent-orange-500"
                checked={form.hasBib ?? false}
                onChange={(e) => set("hasBib", e.target.checked)}
              />
              <div>
                <span className="text-white text-sm">🏅 Pettorale</span>
                <p className="text-gray-500 text-xs">Ho già il pettorale / mi sono iscritto/a</p>
              </div>
            </label>
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              <input
                type="checkbox"
                className="w-4 h-4 accent-orange-500"
                checked={form.hasHotel ?? false}
                onChange={(e) => set("hasHotel", e.target.checked)}
              />
              <div>
...
export default function EventPage() {
  ...
  const [modal, setModal] = useState<{ open: boolean; participant: Participant | null }>({
    open: false,
    participant: null,
  });
    ...
    <main className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            { label: "Partecipanti", value: stats.total, icon: "🏃" },
            { label: "Pettorale", value: stats.bib, icon: "🏅" },
            { label: "Trasporto", value: stats.transport, icon: "✈️" },
            { label: "Albergo", value: stats.hotel, icon: "🏨" },
          ].map((s) => (
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-bold">Lista partecipanti</h2>
            <button
          ...
          <div className="space-y-2">
            {event.participants.map((p, i) => (
              <button
                ...
                className="w-full bg-gray-900 hover:bg-gray-800 border border-white/10 hover:border-orange-500/30 rounded-xl p-4 text-left transition-all"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="text-gray-600 text-sm w-6 text-right">{i + 1}</span>
                    <span className="font-medium text-white">{p.name}</span>
                    {p.notes && <span className="text-gray-500 text-xs truncate max-w-32">{p.notes}</span>}
                  </div>
                  <span className="text-gray-600 text-xs">modifica →</span>
                </div>
                <div className="flex gap-2 mt-2 ml-9">
                  <StatusBadge active={p.hasBib} label="Pettorale" />
                  <StatusBadge
                    active={p.hasTransport}
                    label={p.hasTransport && p.transportType ? (p.transportType === "treno" ? "Treno" : "Volo") : "Trasporto"}
                  />
                  <StatusBadge active={p.hasHotel} label="Albergo" />
                </div>
              </button>
            ))}
          </div>
